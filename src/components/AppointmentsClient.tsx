...
export default function AppointmentsClient({
  ...
  const toMinutes = (iso: string) => {
    ...
    const timeStr = d.toLocaleTimeString('es-CR', { hour: '2-digit', minute: '2-digit', hour12: false, timeZone: timezone })
            ...
            return (
              <div key={d.toISOString()} className="apt-day-col" style={{ height: gridHeight }}>
                ...
                {dayAppts.map(({ appt, col, cols }) => {
                  ...
                  const startTimeStr = start.toLocaleTimeString('es-CR', { hour: '2-digit', minute: '2-digit', hour12: false, timeZone: timezone })
                  const [startHours, startMins] = startTimeStr.split(':').map(Number)
                  const startMin = startHours * 60 + startMins - START_HOUR * 60
