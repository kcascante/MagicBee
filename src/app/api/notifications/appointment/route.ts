...
export async function POST(req: Request) {
  try {
    ...
    if (event === 'status_update') {
      if (!client?.email) return NextResponse.json({ skipped: true, reason: 'no client email' })
      ...
      if (appt.status === 'confirmed') {
        ...
        const html = wrapper(
          ...
          `<h2 style="margin-top: 0;">¡Hola ${client.full_name}!</h2>
           ...
           ${detailsTable(baseRows)}
           <a href="${portalUrl}" style="display: block; text-align: center; background: ${accent}; color: #fff; text-decoration: none; padding: 12px; border-radius: 10px; font-weight: 600; margin-top: 16px;">Agendar otra cita</a>
           <p style="color: #aaa; font-size: 12px; margin-top: 24px;">Enviado por MagicBee en nombre de ${org.name}.</p>`
        )
        results.client = await resend.emails.send({
          from: FROM,
